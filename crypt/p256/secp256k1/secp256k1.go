...
import (
	"errors"
	"github.com/warp-contracts/sequencer/crypt/p256/utils"
	"unsafe"
)
...
func init() {
	ctx = C.warp_secp256k1_context_create(C.SECP256K1_CONTEXT_SIGN | C.SECP256K1_CONTEXT_VERIFY)
...
func SeckeyVerify(seckey []byte) bool {
	return C.warp_secp256k1_ec_seckey_verify(ctx, cBuf(seckey)) == 1
}
...
func GetPublicKey(seckey []byte) ([]byte, error) {
	...
	result := int(C.warp_secp256k1_ec_pubkey_create(ctx, &pubkey, cBuf(seckey)))
	if result != 1 {
	...
	result = int(C.warp_secp256k1_ec_pubkey_serialize(ctx, &output[0], &outputLen, &pubkey, C.SECP256K1_EC_UNCOMPRESSED))
	if result != 1 {
		return nil, ErrGetPublicKeyFailed
	}
...
func RecoverECDSAPublicKey(msg []byte, signature []byte) ([]byte, error) {
	...
	result := int(C.warp_secp256k1_ecdsa_recoverable_signature_parse_compact(ctx, &sig, (*C.uchar)(unsafe.Pointer(&signature[0])), (C.int(signature[64]))))
	if result != 1 {
		return nil, ErrRecoverFailed
	}
	if int(C.warp_secp256k1_ecdsa_recover(ctx, &pubkey, &sig, cBuf(msg))) != 1 {
		return nil, ErrRecoverFailed
	}
	output := make([]C.uchar, 65)
	outputLen := C.size_t(65)
...
func Sign(msg []byte, seckey []byte) ([]byte, error) {
	...
	if C.warp_secp256k1_ec_seckey_verify(ctx, cBuf(seckey)) != 1 {
		return nil, ErrInvalidPrivateKey
	}

	var (
		noncefunc = C.warp_secp256k1_nonce_function_rfc6979
		sigstruct C.secp256k1_ecdsa_recoverable_signature
	)
	if C.warp_secp256k1_ecdsa_sign_recoverable(ctx, &sigstruct, cBuf(msg), cBuf(seckey), noncefunc, nil) == 0 {
	...
	C.warp_secp256k1_ecdsa_recoverable_signature_serialize_compact(ctx, cBuf(sig), &recid, &sigstruct)
...
func Verify(msg []byte, signature []byte, pub []byte) (bool, error) {
	...
	result := int(C.warp_secp256k1_ec_pubkey_parse(ctx, &pubkey, cBuf(pub), C.size_t(len(pub))))
	if result != 1 {
		return false, ErrInvalidPublicKey
	}
	result = int(C.warp_secp256k1_ecdsa_signature_parse_compact(ctx, &sig, cBuf(signature)))
	if result != 1 {
		return false, ErrInvalidSignature
	}
	result = int(C.warp_secp256k1_ecdsa_verify(ctx, &sig, cBuf(msg), &pubkey))
	return result == 1, nil
}
