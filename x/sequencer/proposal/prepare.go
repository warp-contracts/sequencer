...
func NewPrepareProposalHandler(provider *sequencer.ArweaveBlockProvider, txConfig client.TxConfig) sdk.PrepareProposalHandler {
...
func (h *prepareProposalHandler) prepare(ctx sdk.Context, req *abci.RequestPrepareProposal) (*abci.ResponsePrepareProposal, error) {
	// For logging
	now := time.Now()
	...
	if nextArweaveBlock != nil &&
		...
		arweaveBlockTx, err := h.createArweaveTx(ctx, nextArweaveBlock, prevSortKeys)
		if err != nil {
			return nil, err
		}
		...
		if size > req.MaxTxBytes {
			return nil, fmt.Errorf("MaxTxBytes limit (%d) is too small! It is smaller than the size of the Arweave block (%d)",
				req.MaxTxBytes, size)
		}
	} else {
	...
	for i := range req.Txs {
		// Fill in helper data
		txBytes, err := h.prepareDataItem(ctx.BlockHeader().LastBlockId.Hash, req.Txs[i], sortKey, prevSortKeys)
		if err != nil {
			return nil, err
		}
...
func (h *prepareProposalHandler) createArweaveTx(ctx sdk.Context, nextArweaveBlock *types.NextArweaveBlock, prevSortKeys *PrevSortKeys) ([]byte, error) {
...
func (h *prepareProposalHandler) prepareDataItem(sequencerBlockHash []byte, txBytes []byte, sortKey *SortKey, prevSortKeys *PrevSortKeys) ([]byte, error) {
	// decode tx
